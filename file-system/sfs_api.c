...
int sfs_remove(char *file){ 
    ...
    for(i = 0; i < MAX_FILES; i++){
        ...
        fprintf(stderr, "@error: no filename match found -- check offset or display directory contents\n");
        return -1;
}
/**********************************************************************************************/
...
int sfs_fseek(int fileID, int offset){
    if(fileID >= numFiles || descriptorTable[fileID] == NULL || inodeTable[descriptorTable[fileID]->inode].size < offset ){
        fprintf(stderr, "@error: no file descriptor entry found -- check offset or plug in debugger\n");
        return -1;
    }
...
int sfs_fopen(char *name){
    ...
    for(i = 0; i < MAX_FILES; i++){
        if(strncmp(rootDir[i].filename, name, MAXFILENAME + 1) == 0 ){
            if(rootDir[i].inode == 5000){
                fprintf(stderr, "@error: restricted inode reached -- check leak or scrap filesystem and re-initialize\n");
                return -1;
            }
    ...
    for(i = 0; i < MAX_FILES; i++){
        if(strncmp(rootDir[i].filename, "\0", 1) == 0 && rootDir[i].inode == 5000){
            ...
            if(newEntry == NULL){
                fprintf(stderr, "@error: new file creation failed -- no file descriptor entry found\n");
                return -1;
            }
            ...
            if(inode == -1){
                fprintf(stderr, "@error: maximum inode no. reached -- scrap filesystem and re-initialize\n");
                return -1;
            }
